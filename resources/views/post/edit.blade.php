 ...
 <div class="col-md-9 col-lg-7 bg-white shadow-sm p-3 mb-5 bg-white rounded">
   <form action="{{ $post->path() }}" method="POST">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" id="title" name="title" placeholder="Enter title here..." value="{{ $post->title }}" required>
      </div>
      <div class="form-group">
        <label for="body">Description</label>
        <textarea class="form-control {{ $errors->has('body') ? ' is-invalid' : '' }}" id="body" name="body" rows="3" placeholder="Write description here..." required>{{ $post->body }}</textarea>
      </div>
